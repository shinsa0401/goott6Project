...
public class FreeUploadFileProcess {
   /**
    ...
    * 이미지 파일이라면 썸네일 이미지를 만들어 썸네일이미지와 원본 파일을 함께 저장
    * 이미지 파일이 아니라면 원본 파일만 저장
    * 
    * 
    */
   public static FreeUploadFile uploadFileProcess(String upPath, String originalFileName, byte[] file, String contentType) throws IOException {
      ...
      FreeUploadFile upFile = new FreeUploadFile();
      ...
      if(FreeImgMediaConfirm.getMediaType(contentType.toLowerCase()) != null) { // 이미지 파일이면....
         ...
         String thumbnailImageFileName =  saveFileName; // 저장될 썸네일 이미지 파일 이름
    ...
    * 실제 파일이 저장되는 경로 하위에 업로드된 년/월/일을 얻어와 해당 날짜의 폴더를 만들 수 있도록 함
    */
    ...
    * @author : webshjin
    * @date : 2022. 9. 13.
    * @입력 param : upPath경로 밑에 yearPath, monthPath, datePath의 폴더를 각가 만든다
    * @returnType : void
    */
   private static void makeDir(String upPath, String... paths) {
      // String... : String 타입의 매개변수로 가변인자 배열로 받을것임을 컴파일러에게 알려줌. yearPath, monthPath, datePath 값을 paths라는 이름의 배열로
      // 넘겨주게 된다
      
      if(new File(upPath + paths[paths.length - 1]).exists()) { // \2022\09\13 가 있냐?

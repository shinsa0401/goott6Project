...
import com.boritgogae.board.question.etc.IPCheckQuestion;
...
import com.boritgogae.board.question.persistence.BoardQuestionDAO;
...
public class BoardQuestionServiceImpl implements BoardQuestionService {
	...
	public Map<String, Object> viewAllBoard(int pageNo, SearchCriteriaQuestion sc) throws Exception {
		...
		PagingInfoQuestion pi = pagingProcess(pageNo, sc);
		
		List<BoardQuestionVo> lst = null;
		...
		if (row == 1) {
			...
			if (dao.updateRef(lastNo) == 1) {
				...
				if (uploadFileLst.size() > 0) {
					for (UploadFileQuestion up : uploadFileLst) {
						if (up.isImage()) {
	...
	public Map<String, Object> viewBoard(int no) throws Exception {
		...
		String ipAddr = IPCheckQuestion.getIpAddr();
		...
		// 글을 조회했던 기록을 검색
		ReadCountQuestionVo rc = dao.getLastReadDate(readCount);
		...
		// 게시글 정보 얻어오기
		BoardQuestionVo board = dao.getBoard(no);
		// 첨부파일 얻어오기
		List<UploadFileQuestionVo> fileList = dao.getAttachFiles(no);
	...
	// 페이징
	private PagingInfoQuestion pagingProcess(int pageNo, SearchCriteriaQuestion sc) throws Exception {
		PagingInfoQuestion result = new PagingInfoQuestion();

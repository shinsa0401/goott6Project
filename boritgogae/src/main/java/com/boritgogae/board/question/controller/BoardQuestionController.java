...
import com.boritgogae.board.question.domain.UploadFileQuestionVo;
import com.boritgogae.board.question.etc.PagingInfoQuestion;
...
import com.boritgogae.board.question.etc.UploadFileProcessQuestion;
import com.boritgogae.board.question.service.BoardQuestionService;
...
public class BoardQuestionController {
	...
	@Inject
	private BoardQuestionService service;
	
	private List<UploadFileQuestion> uploadFileLst = new ArrayList<>();
	...
	public String viewAllBoard(Model model, @RequestParam(value = "pageNo", required = false, defaultValue = "1") int pageNo, RedirectAttributes rttr, SearchCriteriaQuestion sc) throws Exception {
		...
		List<BoardQuestionVo> lst = (List<BoardQuestionVo>) map.get("boardLst");
		PagingInfoQuestion pi = (PagingInfoQuestion) map.get("pagingInfo");
	...
	public String writeBoard(BoardQuestionVo board) throws Exception {
	...
	public String viewBoard(@RequestParam("no") String no, Model model) {
		...
		Map<String, Object> map = new HashMap<>();
		BoardQuestionVo board = null;
		List<UploadFileQuestionVo> fileList = null;
		
		try {
			map = service.viewBoard(bno);
			
			board = (BoardQuestionVo) map.get("board");
			fileList = (List<UploadFileQuestionVo>) map.get("fileList");
	...
	public String modifySave(BoardQuestionVo board) throws Exception {
		String result = "";
	...
	public ResponseEntity<UploadFileQuestion> uploadFile(MultipartFile upFiles, HttpServletRequest request) {
		...
		ResponseEntity<UploadFileQuestion> result = null;
		
		if (upFiles.getSize() > 0) {
			UploadFileQuestion upFile;
			try {
				upFile = UploadFileProcessQuestion.uploadFileProcess(upPath, upFiles.getOriginalFilename(), upFiles.getBytes(), upFiles.getContentType());
	...
	public @ResponseBody String deleteFile(@RequestParam("deleteFileName") String deleteFileName, HttpServletRequest request) {
		...
		for (UploadFileQuestion uf : this.uploadFileLst) {

...
public class PagingInfo {
	...
	private int startNum; // 출력을 시작할 글의 index 번호
	...
	public void setTotalPage(int totalPostCnt) {
		// 전체 페이지 수 구하기 : 전체 글 갯수 / 1페이지당 보여줄 글의 갯수
		// 단, 위의 공식에서 나누어 떨어지지 않는다면 전체 페이지 수 = 나눈 몫 + 1이 된다
	...
	public void setStartNum(int pageNo) {
		//페이징 시작 index 번호 = 페이지당 보여줄 글의 갯수 * (현재페이지 번호 -1)
	...
	public void setTotalPagingBlock(int totalPage) {
		// 전체 페이징 블럭 수 : 전체 페이지 수 / 페이징 블럭(단, 나누어 떨어지지 않으면 + 1)
	...
	public void setStartNumOfCurPagingBlock(int currentPagingBlock) {
		// 현재 페이지의 블럭 시작 번호 : ((현재 페이징 블럭 - 1) * 페이징 블럭 수) + 1
	...
	public void setEndNumOfCurPagingBlock(int startNumOfCurPagingBlock) {
		// 현재 페이지의 블럭 끝 번호 : (현재 페이징 시작번호 + 페이징 블럭) - 1
		this.endNumOfCurPagingBlock = (startNumOfCurPagingBlock + this.pageCntPerBlock) - 1;
		
		// 데이터가 없는 페이지는 페이징 블럭에 포함하지 않게 하기 위해
		if(this.totalPage < this.endNumOfCurPagingBlock) {
			this.endNumOfCurPagingBlock = this.totalPage;
		}
	}

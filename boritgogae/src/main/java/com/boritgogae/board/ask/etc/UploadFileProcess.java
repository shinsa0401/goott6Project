...
public class UploadFileProcess {
	public static UploadFile uploadFileProcess(String upPath, String originalFileName, byte[] file, String contentType) throws IOException{
		...
		String saveFileName = uuid.toString() + "_" + originalFileName; // 중복되지 않는 이름
		...
		upFile.setOriginFileName(savePath + File.separator + saveFileName);
		
		File originTarget = new File(upPath + savePath, saveFileName);
		FileCopyUtils.copy(file, originTarget); // 원본파일 저장
		
		if(ImageMediaConfirm.getMediaType(contentType.toLowerCase())!=null){ // 이미지 파일일 시
			System.out.println("이미지 파일!");
			upFile.setImage(true);
			String ext = originalFileName.substring(originalFileName.lastIndexOf(".") + 1); // 파일 확장자 꺼내기
			
			BufferedImage originImgFile = ImageIO.read(originTarget); // 업로드된 이미지 파일의 원본 읽기

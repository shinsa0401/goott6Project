...
import com.boritgogae.board.market.service.MarketReplyService;
...
public class MarketReplyController {
	...
	private static final Logger logger = LoggerFactory.getLogger(MarketReplyController.class);
	...
	public ResponseEntity<String> replyWrite(@RequestBody MarketReplyVO reply) {
	...
	public ResponseEntity<List<MarketReplyVO>> getEntireReplies(@PathVariable("bno") int bno) {
		System.out.println(bno + "번 글의 모든 댓글을 얻어오자");
		
		ResponseEntity<List<MarketReplyVO>> result = null;
		
		try {
			List<MarketReplyVO> lst = service.getAllReply(bno);
			if (lst.size() < 1) {
				result = null;
			} else {
				result = new ResponseEntity<List<MarketReplyVO>>(lst, HttpStatus.OK);
			}
	...
	public ResponseEntity<String> modiReply(@RequestBody MarketReplyVO reply, @PathVariable("rno") int rno ) {
		ResponseEntity<String> result = null;
		reply.setRno(rno);
	...
	public ResponseEntity<String> delReply(@RequestBody MarketReplyVO reply)  {
		ResponseEntity<String> result = null;
